...
> **2026-07 追記**: `PhilArchive` / `PhilPapers`（および Gutenberg のサードパーティ API `gutendex.com`）は、robots.txt で ClaudeBot を名指しで Disallow しており、Cloudflare 側でも GitHub Actions の IP レンジ自体がブロックされていることを確認した。ボット除外を明示しているサイトを回避策で突破するのは避け、論文系は `Crossref`、書籍系は `Internet Archive` に情報源を切り替えた。現在の実装は `scrape.yaml` を参照。
...
初期保存先は `inbox/` とする。人間または AI が内容確認した後、著作別ノートも研究動向ノートも `notes/西洋哲学/` や `notes/東洋哲学/` 配下へ、Front Matter の `種別`（書籍／研究動向）で区別して再編集する。
...
- `inbox/` の未処理スクレイピング結果を `notes/` に要約する
- 既存ノートと関連するファイルを探す
...
8. 必要に応じて人間または AI が notes/西洋哲学/ または notes/東洋哲学/ に整理し、書籍ノート・研究動向ノートを 種別 で区別する
```

## 運用ルール

- `scrape.yaml` は人間が編集する収集設定ファイル（論文・書籍を一本化）

...
目的は、ユーザーが `research-scrape.yaml` に関心キーワードを記述するだけで、Go 製スクレイパーが関連情報を収集し、Obsidian Vault に Markdown として蓄積し、Codex や Claude Code がその Vault を読んで回答・比較・要約できる状態を作ること。
...
人間が関心領域を指定する層。設定ファイルは `research-scrape.yaml` とする。
...
- `inbox/` と `.scrapem/seen-research-urls.json` だけを最小単位でコミットできる
...
scripts/scrape.sh
  ↓
inbox/ と .scrapem/seen-research-urls.json に変更があれば git commit
  ↓
...
- `inbox/`
- `.scrapem/seen-research-urls.json`
...
services:
  research-scraper:
    ...
    volumes:
      - .:/vault
    command: ["scrapem", "run", "--config", "/vault/research-scrape.yaml"]
  ...
  research-scheduler:
    ...
    volumes:
      - .:/vault
    command: ["scrapem", "watch", "--config", "/vault/research-scrape.yaml"]
```

`research-scraper` は単発実行用、`research-scheduler` は定期実行用とする。開発初期は cron コンテナや外部ジョブ管理を増やさず、Go アプリ側に `watch` サブコマンドを持たせる構成が単純。
...
- `config`: `research-scrape.yaml` の読み込みと検証
...
```text
1. ユーザーが research-scrape.yaml にキーワードを書く
2. Docker Compose で research-scraper または research-scheduler を起動する
...
- `research-scrape.yaml` は人間が編集する論文収集設定ファイル（書籍は `book-scrape.yaml`）
- 自動収集結果はまず `inbox/` に入れる
- 既存の正式ノートをスクレイパーが直接上書きしない
- スクレイピング結果の自動同期は `inbox/` と `.scrapem/seen-research-urls.json` のみに限定する

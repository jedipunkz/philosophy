---
source: "https://archive.org/details/TEST-IGNORE-Perl-cheatsheets"
title: "Test files. Please ignore."
author: "Not me."
year: ""
captured_at: "2026-08-09T02:37:01Z"
updated_at: "2026-08-09T02:37:01Z"
capture_tool: "scrapem-book"
source_name: "archive"
keyword: "ショーペンハウアー"
query: "Arthur Schopenhauer"
plain_text_url: "https://archive.org/download/TEST-IGNORE-Perl-cheatsheets/EssentialPerl_djvu.txt"
public_domain: true
subjects:
tags:
  - "近代哲学"
...
# Test files. Please ignore.

- 著者: Not me.
- 情報源: [archive](https://archive.org/details/TEST-IGNORE-Perl-cheatsheets)
- パブリックドメイン: ✓
...
variable names begin with a dollar sign ($) such as $ sum or $greet ing. Scalar and other
...
using the "defined" operator. Undef looks like when used as a number, or the empty string ""
...
$array[0] = $array[0] + $array[l]; ## $array[0] is now 3
...
$len = garray; ## $len is now 4 (the length of @array)
...
history of readable code. At least I haven't showed you the even more vulgar forms such as ( +
...
## assigns $x=l, $y=2, $z="hello", and the 4 is discarded
...
o shift (array) — returns the frontmost element and removes it from the array.
...
elements,
o unshift (array, elem) — inserts an element at the front of the array.

Opposite of shift.

• Operating at the "back" ($array[$len-l]) end of the array...

o pop (array) — returns the endmost element (right hand side) and removes it

from the array,
o push (array, elem) — adds a single element to the end of the array. Opposite of
...
"false" keywords in the language. Instead, the empty string, the empty array, the number and
undef all evaluate to false, and everything else is true. The logical operators &&, II,! work as in
...
As an alternative to the classic i f ( ) { } structure, you may use if, while, and unless as
...
open(Fl, "filename"); ## open "filename" for reading as file handle

Fl

open(F2, ">f ilename") ; ## open "filename" for writing as file handle

F2

open(F3, ">>appendtome" ) ## open "appendtome" for appending

close (Fl); ## close a file handle
...
open(F4, "Is -1 I"); ## open a pipe to read from an Is process
...
• \w — (lowercase w) matches a "word" character: a letter or digit [a-zA-ZO-9]
...
"pl23g" =~ /p\d\d\dg/ ==> TRUE \d = 0..9 digit
...
• ? — match or 1 occurrences of the pattern to its left

• * — or more occurrences of the pattern to its left

• + — 1 or more occurrences of the pattern to its left

• I — (vertical bar) logical or — matches the pattern either on its left or right
...
"pl2 3g" =~ m/p\d\d\dg/ ==> TRUE

"pl2 3g" =~ m/p\d\d\d\d/ ==> FALSE

#### \w = letter or digit

"pl2 3g" =~ m/\w\w\w\w\w/ ==> TRUE
...
"cat" =~ m/ A (cat |hat)$/ ==> TRUE

"hat" =~ m/ A (cat |hat)$/ ==> TRUE

Essential Perl Page: 16

"cathatcatcat" =~ m/ A (cat |hat) +$/ ==> TRUE

"cathatcatcat" =~ m/ A (c|a| t |h) +$/ ==> TRUE

"cathatcatcat" =~ m/ A (c|a| t) +$/ ==> FALSE
...
through "z". So the \w code is equivalent to [a-zA-ZO-9]. If the first character in a character class is
a caret ( A ) the set is inverted, and matches all the characters not in the given set. So [ A 0-9] matches
...
"bill.gates_emporer@microsoft.com" =~ m/ A [\w._-] +@ [\w._-] +$/ ==> TRUE
...
## if the above matches, $l=="this", $2=="and", $3=="that"
...
string, $ v (dollar-back-quote) = the string before what was matched, and $' (dollar-quote) = the
...
useribillg host :microsoft . com all :billg@microsoft . com
...
The old way to skip everything up until a certain character, say }, uses the [ A }] construct like this...

m/{([ A }]*)}/ ## the inner [ A }] matches any char except }
...
The index (string, string-to-look-f or, start-index) operator searches the first
string starting at the given index for an occurrence of the second string. Returns the based index
...
SubstrQ starts at the given index and continues for the given length.
...
split (As*, \s*/, "dress , betrayal , legacy") ## returns the
array
...
split (As*, \s*/, "dress , betrayal , legacy,", -1) ## returns the
array
...
my ($a, $b) = (1, "hello"); ## declare $a and $b, and assign $a=l,
...
Unix environment). System returns when the program successfully completes, or on error the
global variable $ ? should be set to an error description.

system("mail nick < tmp.txt") == | die "system error $?";

The file-open function can also be used to run a program — a vertical var ( I ) at the end of the
filename runs the filename as a process, and lets you read from its output...

open (F, "Is -1 |"); ## run the Is -1 process, and name its

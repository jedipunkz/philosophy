...
scrape.yaml
  ↓
Go Scraper（論文: PhilArchive / PhilPapers / arXiv、書籍: Wikipedia ja / SEP / Project Gutenberg）
  ↓
...
- **PhilArchive / PhilPapers / arXiv**: 論文・草稿のメタデータ・Abstract・PDF 本文。`## PDF Text` に抽出本文を保存
...
- **Project Gutenberg**: パブリックドメイン書籍の**フルテキスト**（英訳）。古典のみ対象。`public_domain: true`
...
`inbox/` は正式ノートではなく、自動収集素材の置き場である。Codex / Claude Code が参照してまとめる場合は `notes/` 配下に整理する。`public_domain: true` のノートは Gutenberg から本文取得が可能。著作権が切れていないものは書誌情報・概要のみ利用する。
